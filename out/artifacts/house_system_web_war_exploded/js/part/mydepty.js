...
function getAll(current,flag) {
	...
	if(flag==0)
		{
		  $(".tablelist").append("<thead><tr align='center'><th width='10%' align='center'>编号</th><th width='25%' align='center'>部门名称</th><th width='35%' align='center'>部门备注</th><th width='15%'>运营部门</th></tr></thead>");
		}
	else
		{
    ...
    $.ajax({
    	url: baseUrl + '/dept',
    	dataType:'json',
    	data:{current:current,pflag:flag,methodName:"query"},
    	type:'post',
    			...
    			if(xx.pid!=null)
    				{
    				    	    ...
    				    	    srt+='<td><a href="#" class="backdel">' + xx.pflag + '</a></td></tr>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '300px' ],
			content : $webName+'/part/upp.html?pid='+pid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('${pageContext.request.contextPath}/part_del.action?pid='+pid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
