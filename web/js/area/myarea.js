...
function getAll(current) {
    ...
    $.ajax({
    	url: baseUrl + '/myarea',
    	dataType:'json',
    	data:{current:current,methodName:"query"},
    	type:'post',
    			...
    			if(xx.aid!=null)
    				{
    				    ...
    				    srt+='<td>'+xx.aname+'</td>';
    				    srt+='<td><a href="#" class="myupp">修改片区</a></td>';
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('${pageContext.request.contextPath}/area_del.action?aid='+aid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
