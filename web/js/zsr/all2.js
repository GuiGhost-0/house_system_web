		...
		if(aid!=0)
			{
					...
					dataType:'json',
					type:'post',
					data:{aid:aid,sid:sid,methodName:"aidNoZeroQuery"},
					async : true,
					success:function(mydata)
					{
		...
		if(sid!=0)
			{
					...
					dataType:'json',
					type:'post',
					data:{aid:aid,sid:sid,methodName:"sidNoZeroQuery"},
					async : true,
					success:function(mydata)
					{
					   $("#hid").append("<option value='-1'>全部</option>"); 
...
function getAll(current,aid,sid,hid) {
    ...
    $.ajax({
    	url:baseUrl + '/mydj',
    	dataType:'json',
    	data:{current:current,aid:aid,sid:sid,hid:hid,methodName:"query"},
    	type:'post',
    	success:function(mydata)
    	{
    		$.each(mydata,function(index,xx){
